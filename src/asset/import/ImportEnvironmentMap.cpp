...
void writePixel(uint8_t* output, const std::array<float, 4>& rgba, ImageFormat format)
{
    ...
    {
        CHECK(format == ImageFormat::Rgba32Float, "invalid EXR data format");
...
AssetId AssetImporter::importEnvironmentMap(const std::filesystem::path& source)
{
    ...
    CHECK(std::holds_alternative<std::vector<float>>(image.pixels),
          "environment map must be EXR: {}",
          source.string());
    ...
    CHECK(image.projection.has_value() || isLatLongShape || isCubeShape,
          "EXR '{}' has no envmap attribute and is neither 2:1 nor 1:6",
          source.string());
    ...
    CHECK((projection == LoadedImage::Projection::LatLong && isLatLongShape) ||
          ...
          "EXR '{}' dimensions disagree with its envmap layout",
          source.string());
    ...
    radiance.format = image.format;
    radiance.layout = TextureDesc::Layout::Cubemap;
    radiance.width = side;
    radiance.height = side;

    const uint64_t byteCount = static_cast<uint64_t>(side) * side * 6 *
        texture_io::bytesPerPixel(image.format);
    ...
    const size_t bytesPerPixel = texture_io::bytesPerPixel(image.format);
    const float* decoded = std::get<std::vector<float>>(image.pixels).data();
    for (uint32_t face = 0; face < 6; ++face)
    {
        for (uint32_t y = 0; y < side; ++y)
        {
            for (uint32_t x = 0; x < side; ++x)
            {
                ...
                writePixel(output, rgba, image.format);
            }
        }
    }
